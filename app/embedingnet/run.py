...
import look_for_matches

def main(curr_path="./0000731.jpg"):
    datadir = "./data"
    ...
    transformedImage = transform(cloth)
    ims=Variable(transformedImage)
    cudnn.benchmark = True
    tnet.eval()
    embedding = tnet.embeddingnet(ims.unsqueeze(0)).data
    embs_and_style = [(embedding, 'tops')]
    recomendation_of_bottom = look_for_matches.look_for_matches(embs_and_style, 'bottoms')
    recomendation_of_bag = look_for_matches.look_for_matches(embs_and_style, 'bags')

    results = {
        'bottoms' : recomendation_of_bottom,
        'bags' : recomendation_of_bag
    }
